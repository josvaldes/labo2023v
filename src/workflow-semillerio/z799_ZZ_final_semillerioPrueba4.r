...
PARAM  <- list()
PARAM$experimento  <- "ZZ7990Prueba4"
PARAM$exp_input  <- "HT5511Prueba4"

#Atencion, que cada modelos se procesa con 5 semillas, ajuste a SUS necesidades
PARAM$modelos_rank  <- c( 1, 2, 3, 5, 10, 20 )  #Que modelos quiero, segun su posicion en el ranking e la Bayesian Optimizacion, ordenado por ganancia descendente
PARAM$semillerio  <- 50   # cantidad de semillas a utilizar en el semillerio, podrian ser 50 o 100 para mayor estabilidad
PARAM$semilla_primos  <- 792637  # se utiliza para generar el vector de  PARAM$semillerio  semillas
